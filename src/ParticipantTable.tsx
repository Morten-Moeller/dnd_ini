...
export const ParticipantTable: FC<IParticipantTableProps> = ({ participants, setRound, round }) => {
  ...
  return (
    ...
    {
      participants.map((participant: IParticipant, index)  => <ParticipantButton participant={participant} index={index} turn={turn - 1} />)
    }
    </Flex>
      <Flex justify={'space-between'} style={{width: 300, marginTop: 24}}>
        <Button disabled={round === 1 && turn === 1} onClick={handlePreviousClick}>{t('Previous')}</Button>
